...
Third, a proper transactional email via Resend — sending the audit summary to the user's email after they submit. Currently the email is captured but no confirmation is sent. This closes the loop and builds trust.
...
- **Problem solving: 8/10** — Debugged the params async issue systematically, handled the Anthropic API fallback cleanly, made pragmatic architectural decisions quickly.
